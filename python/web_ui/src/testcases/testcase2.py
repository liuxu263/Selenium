...
from python.web_ui.src.testcases.testdata import data_test_case1
...
class Test2(unittest.TestCase):
    ...
    def test1(self):
        ...
        find(self.driver, "百度首页", "搜索输入框").send_keys(data_test_case1.test2)
        click(self.driver, "百度首页", "百度一下按钮")
        find(self.driver, "百度搜索结果页", "搜索结果")

    def test2(self):
        shot_screen(self, self.test2.__name__, "1")
        find(self.driver, "百度首页", "搜索输入框").send_keys(data_test_case1.test1)
        click(self.driver, "百度首页", "百度一下按钮")
        find(self.driver, "百度搜索结果页", "搜索结果")
